...
from .woocommerce_requests import post_request, get_woocommerce_items,get_woocommerce_item_variants,  put_request, get_woocommerce_item_image, get_woocommerce_categories, create_woocommerce_category, get_woocommerce_brands, create_woocommerce_brand
...
def sync_item_with_woocommerce(item, price_list, warehouse, woocommerce_item=None):
    ...
    # Handle categories - get categories from Item Product Category child table
    if erp_item.product_category:
        category_ids = sync_categories_to_woocommerce(erp_item.product_category)
        if category_ids:
            item_data["categories"] = category_ids

    # Handle brand - sync brand from ERPNext to WooCommerce
    if erp_item.brand:
        brand_data = sync_brand_to_woocommerce(erp_item.brand)
        if brand_data:
            # For WooCommerce 9.4+ native brands
            if 'id' in brand_data:
                item_data["brands"] = [{"id": brand_data['id'], "name": brand_data['name']}]
            # For Perfect Brands plugin compatibility
            else:
                item_data["meta_data"] = item_data.get("meta_data", [])
                item_data["meta_data"].append({
                    "key": "pwb_brand",
                    "value": brand_data.get('slug', erp_item.brand.lower().replace(" ", "-"))
                })
...
def sync_categories_to_woocommerce(item_categories):
    """
    Check if categories exist in WooCommerce and create them if they don't
    Returns a list of category IDs for use in product creation
    """
    if not item_categories:
        return []
    
    # Get all existing WooCommerce categories (cached for performance)
    if not hasattr(frappe.local, 'woocommerce_categories_cache'):
        try:
            wc_categories = get_woocommerce_categories()
            frappe.local.woocommerce_categories_cache = {cat['name'].lower(): cat for cat in wc_categories}
        except Exception as e:
            make_woocommerce_log(title="Error fetching WooCommerce categories", status="Error", 
                method="sync_categories_to_woocommerce", message=str(e), exception=True)
            return []
    
    category_ids = []
    
    for category_row in item_categories:
        category_name = category_row.get('category')
        ...
        if category_name_lower in frappe.local.woocommerce_categories_cache:
            category_ids.append({
                'id': frappe.local.woocommerce_categories_cache[category_name_lower]['id'],
                'name': frappe.local.woocommerce_categories_cache[category_name_lower]['name']
            })
        else:
            # Category doesn't exist, create it
            try:
                new_category = create_woocommerce_category(category_name)
                # Add to cache
                frappe.local.woocommerce_categories_cache[category_name_lower] = new_category
                category_ids.append({
                    'id': new_category['id'],
                    'name': new_category['name']
                })
                make_woocommerce_log(title="Created WooCommerce category", status="Success", 
                    method="sync_categories_to_woocommerce", 
                    message="Created category: {0} with ID: {1}".format(category_name, new_category['id']))
            except Exception as e:
                make_woocommerce_log(title="Error creating WooCommerce category", status="Error", 
                    method="sync_categories_to_woocommerce", 
                    message="Failed to create category {0}: {1}".format(category_name, str(e)), 
                    exception=True)
    
    return category_ids

def sync_brand_to_woocommerce(item_brand):
    """
    Check if brand exists in WooCommerce and create it if it doesn't
    Returns brand data for use in product creation
    """
    if not item_brand:
        return None
    
    # Get all existing WooCommerce brands (cached for performance)
    if not hasattr(frappe.local, 'woocommerce_brands_cache'):
        try:
            wc_brands = get_woocommerce_brands()
            frappe.local.woocommerce_brands_cache = {brand['name'].lower(): brand for brand in wc_brands}
        except Exception as e:
            make_woocommerce_log(title="Error fetching WooCommerce brands", status="Warning", 
                method="sync_brand_to_woocommerce", message=str(e), exception=False)
            return None
    
    brand_name_lower = item_brand.lower()
    
    # Check if brand exists in cache
    if brand_name_lower in frappe.local.woocommerce_brands_cache:
        return frappe.local.woocommerce_brands_cache[brand_name_lower]
    else:
        # Brand doesn't exist, create it
        try:
            new_brand = create_woocommerce_brand(item_brand)
            # Add to cache
            frappe.local.woocommerce_brands_cache[brand_name_lower] = new_brand
            make_woocommerce_log(title="Created WooCommerce brand", status="Success", 
                method="sync_brand_to_woocommerce", 
                message="Created brand: {0} with ID: {1}".format(item_brand, new_brand['id']))
            return new_brand
        except Exception as e:
            make_woocommerce_log(title="Error creating WooCommerce brand", status="Error", 
                method="sync_brand_to_woocommerce", 
                message="Failed to create brand {0}: {1}".format(item_brand, str(e)), 
                exception=True)
            return None    item_code = get_item_code(woocommerce_item, woocommerce_settings)
